            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-10 flex flex-col gap-y-5">
                ...
                <div class="flex flex-row justify-between items-center">
                    <div class="flex flex-col">
                        ...
                        <p class="text-slate-500 text-sm">{{ $course->course_video->count() }} Total Videos</p>
                    </div>
                    <a href="{{ route('admin.courses.add_video', $course->id) }}"
                        class="font-bold py-4 px-6 bg-indigo-700 text-white rounded-full">
                        Add New Video
                    </a>
                </div>

                @forelse($course->course_video as $video)
                    <div class="item-card flex flex-row gap-y-10 justify-between items-center">
                        <div class="flex flex-row items-center gap-x-3">
                        ...
                        <div class="flex flex-row items-center gap-x-3">
                            <a href="{{ route('admin.courses_videos.edit', $video->id) }}"
                                class="font-bold py-4 px-6 bg-indigo-700 text-white rounded-full">
                                Edit Video
                            </a>
                            <form action="{{ route('admin.courses_videos.destroy', $video->id) }}" method="POST">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="font-bold py-4 px-6 bg-red-700 text-white rounded-full">
